...
const toId = (str) =>
  String(str)
    ...
    .replace(/(.)([A-Z])/g, "$1-$2")
    .toLowerCase();
const exclude = (skipped) => (name) =>
  typeof name === "string" && !skipped.some((id) => name.includes(id));
...
const write = (file, text) => fs.promises.writeFile(file, text, "utf-8");
...
module.exports = function ({
  ...
  return {
    ...
    buildStart({ plugins }) {
      ...
      if (terser) {
        optimize = async (code) =>
          (await terser.renderChunk(code, null, {})).code;
      }
    },
    /**
     * Detect used methods, source remains unchanged
